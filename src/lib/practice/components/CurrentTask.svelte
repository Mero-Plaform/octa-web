...
>
  <WordData
    ...
    coverStyles="absolute bg-green-400 w-full transition-all rounded-md leading-7 font-bold text-green-800"
  ...
  <div>
    <span class="chip bg-gray-300 text-white cursor-default !filter-none"
      >hide</span
    >
    <SlideToggle
      name="slide"
      class="bg-gray-300 hover:brightness-95 focus:brightness-95 align-middle mx-2 outline-none"
      active="bg-pink-500"
      bind:checked={showPracticeTarget}
      size="sm"
    />
    <span class="chip bg-pink-500 text-white cursor-default !filter-none"
      >show</span
    >
  </div>
  ...
  <WordData
    show={showPracticeTarget}
    coverStyles="absolute bg-orange-400 w-full transition-all rounded-md leading-7 font-bold text-orange-800"
