<script lang="ts">
  ...
  const checkIsOpen = () => {
    if ($modalStore.length === 0 ) { return false; }
    return $modalStore.find((modals: { type: string, component: string }) => {
      if (modals.type !== 'component') { return false; }
...
<button on:dragstart|preventDefault on:click={openModal} class="btn h-10 w-10 p-2 bg-cyan-400 hover:bg-cyan-500 hover:filter-none rounded-md active:scale-90 active:bg-cyan-500 focus:bg-cyan-500 active:filter-none outline-none"> 
  <div class="h-full w-full bg-white" style={getIconMaskStyes("/src/assets/icons/plus.svg")}></div>
</button>
